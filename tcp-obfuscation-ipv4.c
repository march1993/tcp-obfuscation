...
static int __init init_tcp_obfuscation_ipv4(void) {
  ...
  nf_register_net_hook(&init_net, &tcp_obfuscation_ops_ipv4_incoming);
  nf_register_net_hook(&init_net, &tcp_obfuscation_ops_ipv4_outgoing);
...
static void __exit exit_tcp_obfuscation_ipv4(void) {
  ...
  nf_unregister_net_hook(&init_net, &tcp_obfuscation_ops_ipv4_incoming);
  nf_unregister_net_hook(&init_net, &tcp_obfuscation_ops_ipv4_outgoing);
}
